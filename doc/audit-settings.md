...
# Audit DB Cleaner min/max time before deleting anticipated transitions
audit.globalSettings.range.8 = sonar.dbcleaner.daysBeforeDeletingAnticipatedTransitions, 10, 30, MEDIUM, PERFORMANCE
...
audit.globalSettings.range.20 = sonar.technicalDebt.developmentCost, 20, 30, MEDIUM, CONFIGURATION
...
#----------------------------- PROJECT/APPS/PORTFOLIOS/QG/QP PERMISSIONS ----------------------------
...
audit.projects.permissions.maxIssueAdminGroups = 3
audit.projects.permissions.maxHotspotAdminGroups = 3
...
# Maximum number of differences allowed between 2 similar project keys (levenshtein distance)
# this is limited by half of the key length
audit.projects.duplicates.maxDifferences = 4
...
# Audits projects where the new code is too much code
# Max number of lines allowed in new code (this is lines, not LoC)
# Set property to 0 to turn off the check
...
# Audit for duplicate quality profiles
audit.qualityProfiles.duplicates = yes

# Audit max nbr of quality profiles per language, set to 0 to disable
audit.qualityProfiles.maxPerLanguage = 5
