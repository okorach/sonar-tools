...
def test_measures_export(csv_file: Generator[str]) -> None:
    ...
    assert tutil.run_cmd(measures_export.main, f"{CMD} -{opt.REPORT_FILE_SHORT} {csv_file} --withTags") == e.OK
    assert tutil.csv_col_sorted(csv_file, "key")
    assert tutil.csv_col_match(csv_file, "reliability_rating", _RATING_LETTER)
    assert tutil.csv_col_match(csv_file, "security_rating", _RATING_LETTER)
    assert tutil.csv_col_float_pct(csv_file, "duplicated_lines_density", True)
    assert tutil.csv_col_float_pct(csv_file, "sqale_debt_ratio", True)
    assert not tutil.csv_cols_present(csv_file, "statements", "ncloc_language_distribution")
    ...
    assert tutil.csv_cols_present(csv_file, *col_list)
    for col in col_list:
        assert tutil.csv_col_int(csv_file, col)


def test_measures_conversion(csv_file: Generator[str]) -> None:
    """test_measures_conversion"""
    assert tutil.run_cmd(measures_export.main, f"{CMD} -{opt.REPORT_FILE_SHORT} {csv_file} -r -p --withTags") == e.OK
    assert tutil.csv_col_match(csv_file, "reliability_rating", _RATING_NUMBER)
    assert tutil.csv_col_match(csv_file, "security_rating", _RATING_NUMBER)
    assert tutil.csv_col_pct(csv_file, "duplicated_lines_density", True)
    assert tutil.csv_col_pct(csv_file, "sqale_debt_ratio", True)
...
def test_measures_export_with_url(csv_file: Generator[str]) -> None:
    ...
    if tutil.SQ.edition() == c.CE:
        assert tutil.run_cmd(measures_export.main, cmd) == e.UNSUPPORTED_OPERATION
        return
...
def test_measures_export_json(json_file: Generator[str]) -> None:
    ...
    if tutil.SQ.edition() == c.CE:
        ...
        return
    assert tutil.run_cmd(measures_export.main, cmd) == e.OK
    assert tutil.json_field_sorted(json_file, "key")
    assert tutil.json_field_match(json_file, "reliability_rating", _RATING_LETTER, allow_null=True)
    assert tutil.json_field_match(json_file, "security_rating", _RATING_LETTER, allow_null=True)
    assert tutil.json_field_float(json_file, "duplicated_lines_density")
    assert tutil.json_field_float(json_file, "sqale_debt_ratio")
    assert tutil.json_fields_absent(json_file, "statements", "ncloc_language_distribution")
    col_list = col_list = ("false_positive_issues",)
    if tutil.SQ.version() >= c.MQR_INTRO_VERSION:
        col_list += ("accepted_issues",)
        if tutil.SQ.edition() in (c.EE, c.DCE):
            col_list += ("prioritized_rule_issues",)
    assert tutil.json_fields_present(json_file, *col_list)
    for col in col_list:
        assert tutil.json_field_int(json_file, col)
...
def test_measures_export_json_all(json_file: Generator[str]) -> None:
    ...
    if tutil.SQ.edition() != c.CE:
        cmd += f" -{opt.BRANCH_REGEXP_SHORT} .+"
    assert tutil.run_cmd(measures_export.main, cmd) == e.OK
    assert tutil.json_field_int(json_file, "statements")
    assert tutil.json_fields_present(json_file, "ncloc_language_distribution")
...
def test_measures_export_dateonly(csv_file: Generator[str]) -> None:
    ...
    assert tutil.run_cmd(measures_export.main, cmd) == e.OK
    assert tutil.csv_col_date(csv_file, "lastAnalysis")
...
def test_measures_export_json_dateonly(json_file: Generator[str]) -> None:
    ...
    assert tutil.run_cmd(measures_export.main, cmd) == e.OK
    assert tutil.json_field_date(json_file, "lastAnalysis")
...
def test_specific_measure(csv_file: Generator[str]) -> None:
    ...
    assert tutil.run_cmd(measures_export.main, cmd) == e.OK
    assert tutil.csv_col_int(csv_file, "sqale_index")
    assert tutil.csv_col_float_pct(csv_file, "coverage")
    assert not tutil.csv_cols_present(csv_file, "code_smells", "bugs", "security_rating")
...
def test_non_existing_measure(csv_file: Generator[str]) -> None:
    ...
    assert tutil.run_cmd(measures_export.main, cmd) == e.NO_SUCH_KEY
...
def test_non_existing_project(csv_file: Generator[str]) -> None:
    ...
    assert tutil.run_cmd(measures_export.main, cmd) == e.WRONG_SEARCH_CRITERIA
    assert not tutil.file_exists(csv_file)
...
def test_portfolios_with_tags(csv_file: Generator[str]) -> None:
    ...
    assert tutil.run_cmd(measures_export.main, cmd) == e.ARGS_ERROR
...
def test_option_apps(csv_file: Generator[str]) -> None:
    ...
    assert tutil.run_cmd(measures_export.main, cmd) == e.OK
    assert tutil.csv_col_is_value(csv_file, "type", "APPLICATION")
