...
#
# sonar-tools
# Copyright (C) 2021-2026 Olivier Korach
...
do
    ...
    f="measures-history-${env}-2.csv";     run_test "${f}" sonar-measures-export -b '.+' -k okorach-oss_sonar-tools --history --asTable
    ...
    f="findings-${env}-2.json";     run_test_stdout "${f}" sonar-findings-export -v DEBUG --format json -k '(okorach_audio-video-tools|okorach-oss_sonar-tools)'
    f="findings-${env}-3.json";     run_test_stdout "${f}" sonar-findings-export -v DEBUG --format json -k '(okorach_audio-video-tools|okorach-oss_sonar-tools)' --useFindings
    f="findings-${env}-4.csv";      run_test_stdout "${f}" sonar-findings-export --format csv -k '(okorach_audio-video-tools|okorach-oss_sonar-tools)' --csvSeparator '+'
    ...
    f="config-${env}-1.json";       run_test_stdout "${f}" sonar-config -e -w "qualitygates, qualityprofiles, projects" -k '(okorach_audio-video-tools|okorach-oss_sonar-tools)'
    ...
    f1="findings-${env}-admin.csv";   run_test "${f1}" sonar-findings-export -v DEBUG -k '(okorach_audio-video-tools|okorach-oss_sonar-tools)'
    ...
    if [[ "${env}" = "sonarcloud" ]]; then
        ...
        f2="findings-${env}-user.csv";    run_test "${f}2" sonar-findings-export -v DEBUG -k '(okorach_audio-video-tools|okorach-oss_sonar-tools)'
    fi
