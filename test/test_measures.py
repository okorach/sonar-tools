...
def test_measures_export(get_csv_file: callable) -> None:
    ...
    file = get_csv_file
    util.run_success_cmd(measures_export.main, f"{CMD} {util.STD_OPTS_STR} --withTags -{opt.REPORT_FILE_SHORT} {file}")
...
def test_measures_conversion(get_csv_file: callable) -> None:
    ...
    file = get_csv_file
    util.run_success_cmd(measures_export.main, f"{CMD} {util.STD_OPTS_STR} -r -p --withTags -{opt.REPORT_FILE_SHORT} {file}")
    with open(file=file, mode="r", encoding="utf-8") as fh:
        csvreader = csv.reader(fh)
        line = next(csvreader)
        rating_col_1 = line.index("reliability_rating")
...
def test_measures_export_history_as_table(get_csv_file: callable) -> None:
    ...
    file = get_csv_file
    util.run_success_cmd(measures_export.main, f"{CMD} {util.STD_OPTS_STR} --history --asTable -{opt.REPORT_FILE_SHORT} {file}")
