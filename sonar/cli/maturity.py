...
def get_project_maturity_data(project: projects.Project, settings: dict[str, Any]) -> dict[str, Any]:
    ...
    history = [util.age(sutil.string_to_datetime(d["date"])) for d in project.get_analyses()]
    ...
    for branch in proj_branches:
        history += [util.age(sutil.string_to_datetime(d["date"])) for d in branch.get_analyses()]
