    ...
    def __audit_binding_valid(self, audit_settings):
        ...
        try:
            ...
            if e.response.status_code in (HTTPStatus.BAD_REQUEST, HTTPStatus.NOT_FOUND):
                ...
                util.exit_fatal(f"alm_settings/validate_binding returning status code {e.response.status_code}, exiting", errcodes.ERR_SONAR_API)
...
def __export_zip_thread(queue, results, statuses, export_timeout):
    while not queue.empty():
        ...
        try:
            ...
            queue.task_done()
            util.exit_fatal("Zip export unsupported on your SonarQube version", errcodes.ERR_UNSUPPORTED_OPERATION)
        status = dump["status"]
