...
def parse_and_check(parser: argparse.ArgumentParser, logger_name: str = None, verify_token: bool = True) -> object:
    ...
    try:
        ...
        sys.exit(errcodes.ERR_ARGS_ERROR)
...
def check_token(token: str, is_sonarcloud: bool = False) -> None:
    ...
    if token is None:
        exit_fatal(
            ...
            errcodes.ERR_SONAR_API_AUTHENTICATION,
        )
...
def check_what(what, allowed_values, operation="processed"):
    ...
    for w in what:
        ...
        exit_fatal(
            ...
            exit_code=errcodes.ERR_ARGS_ERROR,
        )
    return what
