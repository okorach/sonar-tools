...
class Branch(components.Component):
    ...
    @classmethod
    def get_object(cls, endpoint: Platform, project_key: str, branch_name: str) -> Branch:
        """Gets a SonarQube Branch object

        :param str project_key: Project key concerned by the branch
        ...
        if self._new_code is None and self.endpoint.is_sonarcloud():
            ...
            for b in data["newCodePeriods"]:
                ...
                if b["branchKey"] == self.name:
                    ...
                    Branch.get_object(endpoint=self.endpoint, project_key=self.concerned_object.key, branch_name=b["branchKey"])._new_code = new_code
