...
class User(SqObject):
    ...
    def create(cls, endpoint: Platform, login: str, name: str, is_local: bool = True, password: Optional[str] = None) -> User:
        ...
        api_def = Api("User", api_mgr.CREATE, endpoint.version())
    ...
    def __get_object_by_id(cls, endpoint: Platform, id: str) -> User:
        ...
        api_def = Api("User", api_mgr.READ, endpoint.version())
    ...
    def groups(self, **kwargs) -> KeyList:
        ...
        if not self.endpoint.is_sonarcloud() and self.endpoint.version() < c.USER_API_V2_INTRO_VERSION:
            ...
            api_def = Api("User", api_mgr.LIST_GROUPS, self.endpoint.version())
            ret = api_mgr.return_field(api_def)
    ...
    def refresh(self) -> User:
        ...
        api_def = Api("User", api_mgr.READ, vers)
        max_ps = api_mgr.max_page_size(api_def)
    ...
    def update_login(self, new_login: str) -> User:
        ...
        api_def = Api("User", api_mgr.UPDATE, self.endpoint.version())
