...
def search(endpoint=None):
    #
    resp = env.get('api/components/search_projects',
        params={'ps': 500, 'filter': 'qualifier%20=%20APP'}, ctxt=endpoint)
    data = json.loads(resp.text)
    if data['paging']['total'] > 500:
        util.logger.critical("Pagination on applications search is not yet supported "
        "and there are more than 500 of them. Will return only 500 first apps")
    objects = {}
    for obj in data['components']:
        objects[obj['key']] = Application(obj['key'], endpoint=endpoint, data=obj)
    return objects
...
def get(key, sqenv=None):
    ...
    if key not in __OBJECTS:
        __OBJECTS[key] = Application(key=key, endpoint=sqenv)
    return __OBJECTS[key]
