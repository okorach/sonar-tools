...
class Application(aggr.Aggregation):
    ...
    def _audit_singleton(self, audit_settings):
        if not audit_settings['audit.applications'] or not audit_settings['audit.applications.singleton']:
            util.logger.debug("Auditing applications is disabled, skipping...")
            return []
    ...
    def _audit_nbr_projects(self, audit_settings):
        problems = []
        if audit_settings['audit.applications.singleton']:
            problems += super()._audit_singleton_aggregation(broken_rule=rules.RuleId.APPLICATION_SINGLETON)
        else:
            util.logger.debug("Auditing singleton applications is disabled, skipping...")
        if audit_settings['audit.applications.empty']:
            problems += super()._audit_empty_aggregation(broken_rule=rules.RuleId.APPLICATION_EMPTY)
        else:
